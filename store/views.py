...
# 1. Endpoint untuk mendapatkan daftar produk dalam JSON (Untuk Flutter)
def product_list_json(request):
    # 1. Ambil parameter dari URL (?q=...&sort=...)
    q = request.GET.get('q', '')
    ...
    # 3. Logika Search (Jika ada query q)
    if q:
        products = products.filter(name__icontains=q)

    # 4. Logika Sort (Jika ada parameter sort)
    if sort == 'price_asc':
