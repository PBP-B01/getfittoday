...
def add_community(request):
    """
    Menangani halaman Add Community.
    """
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                return JsonResponse({
                    ...
                    "redirect_url": reverse('community_list')
                })
            except Exception as e:
                return JsonResponse({"success": False, "error": str(e)}, status=500)
        else:
            return JsonResponse({"success": False, "error": "Form tidak valid", "errors": form.errors}, status=400)
        ...
        if form.is_valid():
            try:
                ...
                return JsonResponse({
                    ...
                    "redirect_url": reverse('community_detail', args=[pk])
                })
            except Exception as e:
                return JsonResponse({"success": False, "error": str(e)}, status=500)
        else:
...
def promote_admin(request, community_id):
    if request.method == 'POST':
        try:
            ...
            if not target_username:
                return JsonResponse({"status": "error", "message": "Username tidak dikirim"}, status=400)
            ...
            try:
                ...
                community.members.add(target_user)
